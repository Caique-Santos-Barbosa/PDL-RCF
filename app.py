        ...
        for email in os.listdir(users_dir):
            ...
            if os.path.exists(foto_path):
                try:
                    ...
                    if face_encodings:  # Verificar se encontrou rostos
                        ...
                        print(f"Nenhum rosto encontrado em {foto_path}")
...
@require_licenca
def abrir_porta():
    # No Replit, sempre usar modo simulação
    if REPLIT_MODE or DISABLE_PORT_CONTROL:
        print("Modo Replit detectado - usando simulação de abertura de porta")
        registrar_acesso("Administrador", "Acesso simulado (ambiente Replit)")
        return jsonify({
            "message": "Acesso registrado (modo simulação - ambiente Replit)",
            "simulation": True,
            "replit_mode": True
        })
